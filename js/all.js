...
$(document).ready(function () {
	...
	function printData(value){
		...
		for (let i = 0; i < value.length; i++) {
			...
			for (let j = 0; j < value[i].compose.length; j++) {

				b = value[i].compose[j].knifeNumber ;
				
				
				if( j==0 ){
					// $('.datalist').append(`<tr><td>${a}</td><td>${b}</td></tr>`) ;

					$('.datalist').append(`<tr id="${a}"><td>${a}</td><td>${b}</td>`) ;

				}else{
					$('.datalist').append(`<tr><td></td><td>${b}</td></tr>`) ;
				}
				// console.log(`長度:${a} / 組合：${b} `);	
			}
	...
	$('.needlength-js').keypress(function (e) { 

		let idNumber, target ;

		let inputNumber = $('.needlength-js').val() ;

		if(e.which == 13){


			if(inputNumber > 58 || inputNumber < 10){	

			}else{
				// console.log( inputNumber );

				idNumber =  '#'+  $('.needlength-js').val() ;


				// target = $(idNumber).offset().top - 180 ;
				target = $(idNumber).position().top ;
